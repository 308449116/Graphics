...
QList<Operator*> Operator::c_tools;
QPointF Operator::c_down;
QPointF Operator::c_last;
quint32 Operator::c_nDownFlags;

DrawShape Operator::c_drawShape = selection;

static Operator selectTool;
...
static void setCursor(ViewGraphics * scene , const QCursor & cursor )
{
...
Operator::Operator(DrawShape shape)
{
    m_drawShape = shape ;
    m_hoverSizer = false;
...
void Operator::mousePressEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
    c_down = event->scenePos();
    c_last = event->scenePos();
}

void Operator::mouseMoveEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
    c_last = event->scenePos();
}

void Operator::mouseReleaseEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
...
void Operator::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{

}

Operator *Operator::findTool(DrawShape drawShape)
{
    QList<Operator*>::const_iterator iter = c_tools.constBegin();
...
void Operator::mousePressEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
    Operator::mousePressEvent(event,scene);
...
void Operator::mouseMoveEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
    Operator::mouseMoveEvent(event,scene);
    QList<QGraphicsItem *> items = scene->selectedItems();
    AbstractShape * item = 0;
    if ( items.count() == 1 ) {
        item = qgraphicsitem_cast<AbstractShape*>(items.first());
...
void RotationTool::mousePressEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
    Operator::mousePressEvent(event,scene);
    if ( event->button() != Qt::LeftButton ) return;
...
void RotationTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
    Operator::mouseMoveEvent(event,scene);
    QList<QGraphicsItem *> items = scene->selectedItems();
    if ( items.count() == 1 ){
...
void RotationTool::mouseReleaseEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
    Operator::mouseReleaseEvent(event,scene);
    if ( event->button() != Qt::LeftButton ) return;
...
void RectTool::mousePressEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
...
void RectTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
...
void RectTool::mouseReleaseEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
...
void PolygonTool::mousePressEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
    Operator::mousePressEvent(event,scene);
...
void PolygonTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
    Operator::mouseMoveEvent(event,scene);
    setCursor(scene,Qt::CrossCursor);
...
void PolygonTool::mouseReleaseEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
    Operator::mousePressEvent(event,scene);
    if ( c_drawShape == line ){
...
void PolygonTool::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event, ViewGraphics *scene)
{
    Operator::mouseDoubleClickEvent(event,scene);
    item->endPoint(event->scenePos());
    item->updateCoordinate();
    emit scene->itemAdded( item );
    item = NULL;
